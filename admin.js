...
async function gerarLinksElenco() {
    ...
    for (const p of participacoes) {
        ...
        const url = `${window.location.origin}/bilhetes.html?peca=${currentPecaId}&token=${result.token}`;
...
async function loadTurmasConvidadas(pecaId) {
    ...
    try {
        ...
        container.innerHTML = tokens.map(t => {
            const url = `${window.location.origin}/bilhetes.html?peca=${pecaId}&token=${t.token}`;
